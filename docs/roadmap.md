...
- V3.0：最小 Mock WMS SQLite 数据层与 CLI 已落地
- V3.1：最小 ROS 2 task executor 与顺序 runner 已落地，作为当前主线的轻量任务执行验证入口
- V3.1：最小 Mock WMS HTTP API create/query 闭环已完成，当前只暴露 SQLite 数据层，不接 Nav2 execute
- V3.3：live ROS / Nav2 顺序任务执行验证已完成，`station_a` 单条 execute 与 `station_a -> station_b` 顺序 execute 均已拿到真实 `SUCCEEDED`
...
- 已完成：V2.1 / V2.2 测试收口、fixed task points 重复验证、WMS readiness 证据整理
- 已完成：V3.3 live ROS / Nav2 顺序任务执行验证，`station_a`、`station_b` 已通过顺序 execute 真实验证
- 已完成：最小 Mock WMS HTTP API create/query 闭环
- 未来计划：更完整调度层和更结构化的任务流 / 上层调度体系
