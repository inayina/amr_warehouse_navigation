...
def _select_task_columns() -> str:
    return ',\n                '.join(TASK_COLUMNS)


def _fetch_task_row(connection: sqlite3.Connection, task_id: int) -> dict[str, object]:
...
def initialize_database(db_path: Path | None = None) -> Path:
    ...
    with sqlite3.connect(db_file) as connection:
        ...
        if 'status_reason' not in columns:
            connection.execute('ALTER TABLE tasks ADD COLUMN status_reason TEXT')
        connection.execute(
...
def create_task(
    ...
    with sqlite3.connect(db_file) as connection:
        ...
        row = _fetch_task_row(connection, task_id)
        connection.commit()

    return row


def get_next_pending_task(db_path: Path | None = None) -> dict[str, object] | None:
    db_file = initialize_database(db_path)
...
def update_task_status(
    ...
    *,
    ...
    if status not in TASK_STATUSES:
        raise ValueError(
            f'Unsupported task status {status!r}. Allowed values: {", ".join(TASK_STATUSES)}.'
        )
